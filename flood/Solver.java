...
class Solver {
    ...
    static int movesNeeded(Model model) {
        ...
        while (!work.solved()) {
            num += 1;
            int bestMove = chooseBestMove(work);
            work.setActiveRegionColor(bestMove);
        }
        return num;
    }
    ...
    static int[] chooser(Model model, int depth) {
        ...
        bestMove = -1;
        maxSize = -1;
        for (int x = 0; x < model.ncolors(); x += 1) {
            int possibleRes = model.adjacentCells(x).size();
            if (maxSize < possibleRes && x != model.get(0, 0)) {
                maxSize = possibleRes;
                bestMove = x;
            }
        }
        return new int[] { bestMove, maxSize + model.activeRegionSize() };
    }
