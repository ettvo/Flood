...
class Model {
    ...
    Model(Model model) {
        ...
        _history.add(new GameState());
        _history.get(0).saveState();
        int maxHistory = model._lastHistory + 1;
        for (int x = 1; x < maxHistory; x += 1) {
            _history.add(model._history.get(x));
            _history.get(x).saveState();
        }
    }
    ...
    /** Set the move limit to LIMIT>0. Assumes that markedCells() is empty. */
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        boolean assignLeft, assignRight, assignAbove, assignBelow;
        assignLeft = assignRight = assignAbove = assignBelow = false;
        if (place.row == 0) {
            if (place.col == 0) {
                assignRight = true;
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        Consumer<Place> acc = new Consumer<Place>() {
            @Override
            public void accept(Place place) {
                if (!result.contains(place)) {
                    if (get(place) == color) {
                        result.add(place);
                        forNeighbors(place, this);
                    }
                }
            }
        };
    ...
    void setActiveRegionColor(int newColor) {
        Iterator<Place> curr = _active.iterator();
        Place rep = Place.pl(0, 0);
        while (curr.hasNext()) {
            rep = curr.next();
            _cells[rep.row][rep.col] = newColor;
        }
        _active = sameColorRegion(rep.row, rep.col);
        _current += 1;
    ...
    Set<Place> adjacentCells(int color) {
        Set<Place> adjCells = new HashSet<>();
        HashSet<Place> checked = new HashSet<>();
        Iterator<Place> cells = _active.iterator();
        Consumer<Place> action = new Consumer<Place>() {
            @Override
            public void accept(Place place) {
        ...
        while (cells.hasNext()) {
            Place curr = cells.next();
            forNeighbors(curr, action);
        }
        return adjCells;
    }
